...
#
# SNS Notificate dependency Boto3.client (iskim)
DOCUMENTATION = """
---
module: ec2_asg
...
options:
  ...
  sns_topic:
    description:
        - Notificate SNS Topic for Auto Scaling event
    required: false
    default: None
    dependency: Boto3.client
  notification_types:
    description:
        - Notificate Type for Auto Scaling event
    required: false
...
import re # use for Notificate Descript
...
try:
    ...
    from boto3 import client # use for Notificate Descript
    HAS_BOTO = True
except ImportError:
    HAS_BOTO = False
...
def create_autoscaling_group(connection, client, module):
    ...
    sns_topic = module.params.get('sns_topic')
    ...
    if not as_groups:
        ...
        try:
            ...
            if sns_topic is not None:
                ag.put_notification_configuration(sns_topic, notification_types)
        ...
        # Strart Check and Change notification for SNS Topic (iskim)
        try:
            response = client.describe_notification_configurations(AutoScalingGroupNames=[group_name])
            group_notification_types = response.get('NotificationConfigurations')
            if len(group_notification_types) > 0:
                group_notification_types.sort()
                group_notification_type_tmp = ''
                for i in group_notification_types:
                    group_notification_type_tmp = group_notification_type_tmp + str(',') + i.get('NotificationType').replace('autoscaling:', '')
                group_notification_type = re.sub(r'^,', '', group_notification_type_tmp)

                notification_types.sort()
                module_notification_type_tmp = '' 
                for j in notification_types:
                    module_notification_type_tmp = module_notification_type_tmp + str(',') + j.replace('autoscaling:', '')
                module_notification_type = re.sub(r'^,', '', module_notification_type_tmp)
 
                if not (group_notification_type == module_notification_type) or not (group_notification_types[0].get('TopicARN') == sns_topic):
                    as_group.delete_notification_configuration(group_notification_types[0].get('TopicARN'))
                    if sns_topic is not None:
                        as_group.put_notification_configuration(sns_topic, notification_types)
                    changed = True 
            elif sns_topic is not None:
                as_group.put_notification_configuration(sns_topic, notification_types)
                changed = True 

        except BotoServerError, e:
            module.fail_json(msg=str(e))
        # End Check and Change notification for SNS Topic (iskim)
...
def main():
    ...
    try:
        ...
        client = boto3.client('autoscaling', region_name = module.params.get('region'))
        if not connection:
    ...
    if state == 'present':
        create_changed, asg_properties=create_autoscaling_group(connection, client, module)
    elif state == 'absent':
